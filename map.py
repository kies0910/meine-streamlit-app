...
def build_map(mode, year):
    ...
    fig.add_trace(go.Choropleth(
        ...
        marker_line_color="rgba(80,80,80,0.6)", marker_line_width=0.5,
    ))
    ...
    fig.add_trace(go.Choropleth(
        ...
        marker_line_color="rgba(100,100,100,0.7)", marker_line_width=0.8,
    ))
    ...
    fig.update_layout(
        ...
        geo=dict(
            ...
            bgcolor="#000000", projection_type="natural earth",
            lonaxis_range=[-25, 80], lataxis_range=[25, 75],
        ),
...
st.set_page_config(page_title="Russian Migration Map", layout="wide")
...
<style>
    ...
    * { font-family: 'Inter', sans-serif !important; }
    ...
    div[data-testid="stRadio"] label { color: #aaaaaa !important; font-family: 'Inter', sans-serif !important; font-size: 13px !important; }
    div[data-testid="stSlider"] p { color: #aaaaaa !important; font-size: 12px !important; }
    .stButton > button {
...
st.markdown(
    ...
    "Where did Russian men go?</h2>",
    unsafe_allow_html=True,
)
st.markdown(
    "<p style='color:#888888;font-size:12px;margin:0 0 12px;'>"
    "Routes and destination countries for Russian men — hover over a country or route to see the share of applications.</p>",
    unsafe_allow_html=True,
)
...
if st.button("About the data", key="map_info_btn"):
    st.session_state.show_map_info = not st.session_state.show_map_info

if st.session_state.show_map_info:
    st.markdown("""
    <div class="info-box">
        <p><strong>What are we looking at?</strong></p>
        <p>The map shows where Russian men applied for asylum or residence permits per EU country. Use the toggle to switch between asylum applications (monthly data, 2017–2025) and residence permits (annual data, 2017–2024). Use the year slider to explore how destination patterns changed over time.</p>
        <br>
        <p><strong>How did we measure it?</strong></p>
        <p>For asylum applications, we used Eurostat's monthly database and aggregated by year and destination country, covering Russian male applicants aged 18–34. For residence permits, we used Eurostat's annual residence permit database, covering Russian male applicants aged 18–29, the age group available in that dataset. The color scale shows each country's share of total applications in a given year. Route lines show the flow from Russia to each destination country, with line thickness proportional to the number of applications.</p>
        <br>
        <p><strong>Note</strong></p>
        <p>The color scale is fixed across all years so changes between years are directly comparable.</p>
    </div>
    """, unsafe_allow_html=True)
...
if mode == "Asylum":
    st.markdown(
        ...
        "Source: Eurostat · Asylum applications (first-time, men aged 18–34) · Color scale fixed across all years.</p>",
        unsafe_allow_html=True,
    )
else:
    st.markdown(
        "<p style='font-size:10px;color:#888888;margin:4px 0;'>"
        "Source: Eurostat · Residence permit applications (men aged 18–29) · Color scale fixed across all years.</p>",
        unsafe_allow_html=True,
    )
